...
namespace YearChart
{
    ...
    public sealed class ChartViewToolStripButton : ToolStripButton
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            var outlineColor = Enabled ? Color.FromArgb(45, 45, 45) : SystemColors.GrayText;
            var accentColor = Enabled ? Color.FromArgb(0, 120, 215) : SystemColors.GrayText;
        ...
        private static void DrawStretchIcon(Graphics graphics, Rectangle bounds, Color outlineColor, Color accentColor)
        {
            ...
            using var arrowPen = new Pen(accentColor, 1.8f);
        ...
        private static void DrawArrow(Graphics graphics, Pen pen, Point start, Point end)
        {
            ...
            var horizontal = end.X < start.X ? 1 : -1;
            var vertical = end.Y < start.Y ? 1 : -1;
        ...
        {
            var shadowBounds = new Rectangle(bounds.X + 7, bounds.Y + 3, 10, 15);
            var pageBounds = new Rectangle(bounds.X + 4, bounds.Y + 1, 12, 16);
            var marginBounds = new Rectangle(bounds.X + 7, bounds.Y + 5, 6, 8);

            using Brush shadowBrush = new SolidBrush(Color.FromArgb(80, 0, 0, 0));
            using Brush pageBrush = new SolidBrush(Color.White);
            using var pagePen = new Pen(outlineColor, 1.6f);
            using var marginPen = new Pen(accentColor, 1.2f);
